          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 place-items-center gap-5">
            {
              ProductsData.slice(0, visibleCount).map((data)=>(
                <ProductItems key={data.id} data={data} handlePopup={handlePopup}/>
              ))
            }
          </div>
          <div className="flex justify-center">
            {visibleCount < ProductsData.length && <button onClick={() => setVisibleCount(visibleCount + 4)} className="text-center text-white mt-10 cursor-pointer bg-primary py-1 px-5 rounded-md">View All</button> }
          </div>
        </div>
      </div>
    </div>
